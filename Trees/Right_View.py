...
def right_view_recursive(root, level, max_level):
    if(root == None):
        return
    if(max_level[0] < level):
        print(root.data, end=' ')
        max_level[0] = level
    right_view_recursive(root.right, level+1, max_level)
    right_view_recursive(root.left, level+1, max_level)
    return
...
right_view_recursive(root, 0, mx)
...
'''
    Sample Input:
        ...
        10
    Sample Output:
        Iterative Method: 20 30
        Recursive Method: 20 30 
'''
